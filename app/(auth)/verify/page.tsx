...
export default function VerifyPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!res.ok) { const j = await res.json().catch(() => ({})); setErr(j?.error || "Code ungültig."); return }
    ...
    <div className="max-w-sm mx-auto space-y-4">
      ...
      <form onSubmit={onSubmit} className="space-y-3">
        ...
        <div>
          ...
          <input className="w-full h-10 rounded-md border px-3" inputMode="numeric" pattern="\d{6}" maxLength={6} value={code} onChange={e=>setCode(e.target.value)} required />
        </div>
