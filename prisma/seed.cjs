...
async function main() {
  ...
  const q1 = await prisma.question.upsert({
    ...
    create: {
      ...
      type: 'single',
      stem:
        '65-jähriger Patient mit Angina pectoris; welches Medikament ist am geeignetsten?',
  ...
  const q2 = await prisma.question.upsert({
    ...
    create: {
      ...
      type: 'single',
      stem:
        'EKG-Ausschnitt (siehe Bild): welche Diagnose ist am wahrscheinlichsten?',
      explanation: 'Demo-Frage (Platzhalter).',
    },
  });
  ...
  const questions = [
    {
      id: 'q3',
      stem: 'Welche Laborgröße spricht typischerweise für Entzündung?',
      correct: 'CRP',
      options: ['CRP', 'Kreatinin', 'Natrium', 'Kalium'],
    },
    {
      id: 'q4',
      stem: 'Welcher Elektrolyt ist primär intrazellulär?',
      correct: 'Kalium',
      options: ['Kalium', 'Natrium', 'Calcium', 'Chlorid'],
    },
    {
      id: 'q5',
      stem:
        'Standard-Erstlinientherapie bei bakterieller Pneumonie (Platzhalter)?',
      correct: 'Aminopenicillin ± Beta-Laktamase-Inhibitor',
      options: [
    ...
    },
    {
      id: 'q6',
      stem:
        'Welche Maßnahme gehört NICHT zur Reanimation (Platzhalter)?',
      correct: 'Routine-Blutentnahme',
      options: [
        'Thoraxkompressionen',
        'Defibrillation (falls indiziert)',
        'Beatmung',
